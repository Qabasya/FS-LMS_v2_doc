...
| Потребность | Где всплывёт | Кандидат |
|---|---|---|
| Drag-n-drop (дерево курса, полоса шагов, темы → слоты КТП, состав работы) | волна 1, шаг 1.2 | `@dnd-kit/*` — активно поддерживается, дружит с React 18+ |
| Набор иконок интерфейса (меню, действия; Mantine иконок не несёт, в мокапах — юникод-глифы) | волна 0, шаг 0.2 | `@tabler/icons-react` — стандартная пара к Mantine |
| Формулы в редакторе и условиях задач (§1.4) | волна 1, шаг 1.1 | tiptap-расширение + KaTeX — уточнить фиче-спекой конструктора |
