...
**Библиотека работ и экзаменов** — `lms.works`: предмет, режим проведения (`inline / timed / ege` — закрытый вендорский список, §5.3.1), тип работы (справочник школы `work_types` — метка журнала и признак итоговости, поведения не меняет), название, настройки (лимит попыток, лимит времени, показ результата, лог вкладок, шкала оценивания — по режиму); состав — `lms.work_tasks` (работа × задача × позиция).
